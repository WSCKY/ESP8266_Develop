    ...
    while (uart_intr_status != 0x0) {
        if (UART_FRM_ERR_INT_ST == (uart_intr_status & UART_FRM_ERR_INT_ST)) {
            ...
            while (buf_idx < fifo_len) {
                uart_tx_one_char(uart_no, READ_PERI_REG(UART_FIFO(uart_no)) & 0xFF);
                buf_idx++;
            }
...
uart_init_new(void)
{
    ...
    uart_config.baud_rate         = 100000;//BIT_RATE_115200;
    uart_config.data_bits         = UART_WordLength_8b;
    uart_config.parity            = USART_Parity_Even;//USART_Parity_None;
    uart_config.stop_bits         = USART_StopBits_2;//USART_StopBits_1;
    uart_config.flow_ctrl         = USART_HardwareFlowControl_None;
    uart_config.UART_RxFlowThresh = 120;
    uart_config.UART_InverseMask  = UART_Rxd_Inverse;//UART_None_Inverse;
